...
df = pd.read_excel("./src/map/datos/sum_consumo_total.xlsx")
file = open("./src/map/datos/map.geojson", encoding='utf-8')
...
@app.callback(Output("graf", "figure"), Input("tipo", "value"))
def display_choropleth(type):
    df_filt = df.loc[df['TIPO'] == op[type]]
    fig = px.choropleth_mapbox(df_filt,
                               geojson=municipios,
                               featureidkey='properties.CODIGO_INE',
